...
bool ImGui::ButtonExCustom(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float button_width = (size_arg.x > 0.0f) ? size_arg.x : max_width;

    ImVec2 size = CalcItemSize(ImVec2(button_width, size_arg.y), button_width, label_size.y + style.FramePadding.y * 2.0f);
    ...
    ImU32 col_top = IM_COL32(0x14, 0x14, 0x14, 0xFF);
    ImU32 col_bottom = IM_COL32(0x11, 0x11, 0x11, 0xFF);

    if (held && hovered)
    {
        col_top = IM_COL32(0x33, 0x33, 0x33, 0xFF);
        col_bottom = IM_COL32(0x2D, 0x2D, 0x2D, 0xFF);
    }
    else if (hovered)
    {
        col_top = IM_COL32(0x23, 0x23, 0x23, 0xFF);
        col_bottom = IM_COL32(0x20, 0x20, 0x20, 0xFF);
    }

    RenderNavCursor(bb, id);

    window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, col_top, col_top, col_bottom, col_bottom);
    window->DrawList->AddRect(bb.Min, bb.Max, col_border, style.FrameRounding, 1.0f);
...
bool ImGui::ButtonCustom(const char* label, const ImVec2& size_arg)
{
    return ButtonExCustom(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x + 4.0f : 0.0f),
    ...
    if (is_visible)
    {
        ...
        ImU32 col_top = IM_COL32(157, 171, 243, 255);
        ImU32 col_bottom = IM_COL32(90, 98, 136, 255);
        ImU32 grey_top = IM_COL32(20, 20, 20, 255);
        ImU32 grey_bottom = IM_COL32(17, 17, 17, 255);

        ImVec2 fill_min = ImVec2(check_bb.Min.x + 1.0f, check_bb.Min.y + 1.0f);
        ImVec2 fill_max = ImVec2(check_bb.Max.x - 1.0f, check_bb.Max.y - 1.0f);

        if (checked)
        {
            window->DrawList->AddRectFilledMultiColor(fill_min, fill_max, col_top, col_top, col_bottom, col_bottom);
            if (hovered)
            {
                ImU32 overlay_col = IM_COL32(157, 171, 243, (int)(255 * 0.10f));
                window->DrawList->AddRectFilled(fill_min, fill_max, overlay_col);
            }
        }
        else
        {
            window->DrawList->AddRectFilledMultiColor(fill_min, fill_max, grey_top, grey_top, grey_bottom, grey_bottom);
            if (hovered)
            {
                ImU32 overlay_col = IM_COL32(157, 171, 243, (int)(255 * 0.10f));
                window->DrawList->AddRectFilled(fill_min, fill_max, overlay_col);
        ...
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, IM_COL32(0, 0, 0, 255), style.FrameRounding, 0, 1.0f);
    }

    ImVec4 label_color;
    if (hovered)
        label_color = ImVec4(0.9f, 0.9f, 0.9f, 1.0f);
    else if (checked)
        label_color = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
    else
        label_color = GetStyleColorVec4(ImGuiCol_Text);

    const float label_x = check_bb.Max.x + style.ItemInnerSpacing.x + 4.0f;
    const float label_y = check_bb.Min.y + (square_sz - label_size.y) * 0.5f;
    const ImVec2 label_pos(label_x, label_y);
...
bool ImGui::Dropdown(const char* label, int* current_item, const char* const items[], int items_count) {
    ImGui::Text("%s", label);
    return ImGui::DropdownValue(("##" + std::string(label)).c_str(), current_item, items, items_count);
}

bool ImGui::DropdownValue(const char* label, int* current_item, const char* const items[], int items_count)
{
    ...
    bool menu_opened = storage->GetBool(id, false);

    const char* preview_value = items[*current_item];
    ImVec2 label_size = ImGui::CalcTextSize(preview_value);
    float height = 16.0f;
    ...
    if (pressed)
    {
        ...
        storage->SetBool(id, menu_opened);
    }

    if (menu_opened && ImGui::IsMouseClicked(ImGuiMouseButton_Left))
    {
        ImVec2 mouse_pos = ImGui::GetMousePos();
        if (!bb.Contains(mouse_pos) && !ImGui::IsWindowHovered(ImGuiHoveredFlags_AnyWindow))
        {
            menu_opened = false;
    ...
    ImU32 col_top = IM_COL32(0x14, 0x14, 0x14, 255);
    ImU32 col_bottom = IM_COL32(0x11, 0x11, 0x11, 255);
    ImU32 border_color = IM_COL32(0, 0, 0, 255);

    if (hovered)
    {
        auto lighten = [](ImU32 col, int amount) -> ImU32 {
            ImVec4 c = ImGui::ColorConvertU32ToFloat4(col);
            c.x = ImClamp(c.x + amount / 255.0f, 0.0f, 1.0f);
            c.y = ImClamp(c.y + amount / 255.0f, 0.0f, 1.0f);
            c.z = ImClamp(c.z + amount / 255.0f, 0.0f, 1.0f);
            return ImGui::ColorConvertFloat4ToU32(c);
            };
        col_top = lighten(col_top, 15);
        col_bottom = lighten(col_bottom, 15);
    }

    window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, col_top, col_top, col_bottom, col_bottom);
    window->DrawList->AddRect(bb.Min, bb.Max, border_color, 0.0f);

    ImVec2 text_pos = bb.Min + ImVec2(8.0f, (height - label_size.y) * 0.5f);
    window->DrawList->AddText(text_pos, ImGui::GetColorU32(ImGuiCol_Text), preview_value);
    ...
    if (menu_opened)
    {
        ...
        if (ImGui::Begin((std::string(label) + "popup").c_str(), NULL,
...
bool ImGui::DropdownMultiLabel(const char* label, std::vector<bool>* current_items, const char* const items[], int items_count)
{
    ImGui::Text("%s", label);
    return ImGui::DropdownMulti(("##" + std::string(label)).c_str(), current_items, items, items_count);
}

bool ImGui::DropdownMulti(const char* label, std::vector<bool>* current_items, const char* const items[], int items_count)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static ImRect combo_bb;
    static ImVec2 combo_pos;
    ...
    if (preview_value.empty())
        preview_value = "none";

    ImVec2 label_size = ImGui::CalcTextSize(preview_value.c_str());
    float height = 16.0f;
    float width = ImGui::GetContentRegionAvail().x;
    ...
    {
        bool clicked_outside = !bb.Contains(ImGui::GetMousePos());
        if (clicked_outside && ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !ImGui::IsWindowHovered(ImGuiHoveredFlags_AnyWindow))
            menu_opened = false;
    ...
    ImVec2 text_pos = bb.Min + ImVec2(8.0f, (height - label_size.y) * 0.5f);
    window->DrawList->AddText(text_pos, ImGui::GetColorU32(ImGuiCol_Text), preview_value.c_str());

    if (menu_opened)
    {
        ImGui::SetNextWindowSize(ImVec2(combo_width, 0));
        ImGui::SetNextWindowPos(ImVec2(combo_bb.Min.x, combo_bb.Max.y + 2));

        ImGui::PushStyleColor(ImGuiCol_ChildBg, IM_COL32(32, 32, 32, 255));
        ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0.2f, 0.2f, 0.2f, 1.0f));

        if (ImGui::Begin((std::string(label) + "_popup").c_str(), NULL,
            ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize |
            ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings))
        {
            for (int i = 0; i < items_count; i++)
            {
                bool selected = (*current_items)[i];
                if (ImGui::Selectable(items[i], selected))
                {
                    (*current_items)[i] = !selected;
                }
                if (selected)
                    ImGui::SetItemDefaultFocus();
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImU32 col_top = hovered ? IM_COL32(0x1e, 0x1e, 0x1e, 0xFF) : IM_COL32(0x14, 0x14, 0x14, 0xFF);
    ImU32 col_bottom = hovered ? IM_COL32(0x1a, 0x1a, 0x1a, 0xFF) : IM_COL32(0x11, 0x11, 0x11, 0xFF);
    ...
    // Bordure noire
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, border_color, 0.0f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 slider_pos = window->DC.CursorPos + ImVec2(0, label_size.y + 2.0f);
    const ImRect frame_bb(slider_pos, slider_pos + ImVec2(w, slider_height + style.FramePadding.y * 2.0f));
    ...
    if (temp_input_is_active)
    {
        const bool clamp_input = (flags & ImGuiSliderFlags_ClampOnInput) != 0;
        return ImGui::TempInputScalar(frame_bb, id, label, data_type, p_data, format,
            clamp_input ? p_min : nullptr, clamp_input ? p_max : nullptr);
    }
    ...
    ImU32 col_top_checked = IM_COL32(157, 171, 243, 255);
    ImU32 col_bottom_checked = IM_COL32(90, 98, 136, 255);
    ImU32 col_top_unchecked = IM_COL32(20, 20, 20, 255);
    ImU32 col_bottom_unchecked = IM_COL32(17, 17, 17, 255);

    window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max,
        col_top_unchecked, col_top_unchecked, col_bottom_unchecked, col_bottom_unchecked);

    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ImVec2 fill_end = ImVec2(grab_bb.Max.x, frame_bb.Max.y);
        window->DrawList->AddRectFilledMultiColor(frame_bb.Min, fill_end,
            col_top_checked, col_top_checked, col_bottom_checked, col_bottom_checked);
    }

    if (hovered)
    {
        ImU32 overlay_col = IM_COL32(255, 255, 255, (int)(255 * 0.05f));
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, overlay_col);
    }

    ImU32 border_col = IM_COL32(0, 0, 0, 255);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, border_col);
    ...
    ImVec2 value_pos = ImVec2(frame_bb.Min.x + (frame_bb.GetWidth() - value_size.x) * 0.5f, frame_bb.Min.y + (frame_bb.GetHeight() - value_size.y) * 0.5f);

    float padding_x = 6.0f;
    float padding_y = 4.0f;
    ImVec2 rect_min = value_pos - ImVec2(padding_x, padding_y + 2.0f);
    ImVec2 rect_max = value_pos + value_size + ImVec2(padding_x, padding_y - 0.0f);


    ImU32 rect_col = IM_COL32(30, 30, 30, 255);
    ImU32 rect_border = IM_COL32(0, 0, 0, 255);

    ImVec2 fill_rect_min = rect_min + ImVec2(1.0f, 1.0f);
    ImVec2 fill_rect_max = rect_max;

    window->DrawList->AddRectFilled(fill_rect_min, fill_rect_max, rect_col, 0.0f);
    ImVec2 offset = ImVec2(0.5f, 0.5f);
    window->DrawList->AddRect(rect_min + offset, rect_max + offset, rect_border, 0.0f, 0, 1.0f);

    ImU32 col_outline = IM_COL32(0, 0, 0, 255);
    ...
    for (int i = 0; i < 8; i++)
        window->DrawList->AddText(value_pos + offsets[i], col_outline, value_buf);

    window->DrawList->AddText(value_pos, col_text, value_buf);

    ImGui::RenderText(frame_bb.Min - ImVec2(0, label_size.y + 4.0f), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ImU32 col_top = hovered ? IM_COL32(0x1e, 0x1e, 0x1e, 0xFF) : IM_COL32(0x14, 0x14, 0x14, 0xFF);      // #1e1e1e vs #141414
        ImU32 col_bottom = hovered ? IM_COL32(0x1a, 0x1a, 0x1a, 0xFF) : IM_COL32(0x11, 0x11, 0x11, 0xFF);   // #1a1a1a vs #111111
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // Removed HSV input conversion block here
    ...
    if ((flags & ImGuiColorEditFlags_DisplayRGB) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        static const char* fmt_table_float[2][4] =
        {
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        float button_width = square_sz * 1.7f;
        float button_height = square_sz * 0.7f;

        float avail_width = ImGui::GetContentRegionAvail().x;
        if (avail_width > button_width)
        {
            ImGui::Dummy(ImVec2(avail_width - button_width - 9.f, 0.0f));
            ImGui::SameLine();
        }

        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);

        if (ImGui::ColorButton("##ColorButton", col_v4, flags | ImGuiColorEditFlags_NoBorder, ImVec2(button_width, button_height)))
        {
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                SetNextItemWidth(square_sz * 9.5f);
    ...
    if (value_changed && picker_active_window == NULL)
    {
        if (!value_changed_as_float)
            for (int n = 0; n < 4; n++)
                f[n] = i[n] / 255.0f;
        // Removed HSV-related conversions here
        col[0] = f[0];
        col[1] = f[1];
        col[2] = f[2];
        if (alpha)
    ...
    if ((g.LastItemData.StatusFlags & ImGuiItemStatusFlags_HoveredRect) && !(g.LastItemData.ItemFlags & ImGuiItemFlags_ReadOnly) && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropTarget())
    {
        ...
        // Removed HSV drag & drop conversion here
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float square_sz = GetFrameHeight() * 0.74f;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        ImU32 border_col = active ? IM_COL32(0, 0, 0, 255) : IM_COL32(60, 60, 60, 180);

        ImU32 hover_bg = IM_COL32(0x21, 0x22, 0x29, 255);

        ImU32 gradient_top = IM_COL32(0x9D, 0xAB, 0xF3, 255);
        ImU32 gradient_bottom = IM_COL32(0x5A, 0x62, 0x88, 255);
        ...
        else if (active || (flags & ImGuiSelectableFlags_Highlight))
        {
            col_top = gradient_top;
            col_bottom = gradient_bottom;
        }
        else
        {
            col_top = 0;
        ...
        ImU32 text_color = active ? IM_COL32(0, 0, 0, 255) : ImGui::GetColorU32(ImGuiCol_Text);

        if (g.NavId == id)
        {
            ImGuiNavRenderCursorFlags nav_flags = ImGuiNavRenderCursorFlags_Compact | ImGuiNavRenderCursorFlags_NoRounding;
            if (is_multi_select)
                nav_flags |= ImGuiNavRenderCursorFlags_AlwaysDraw;
            RenderNavCursor(bb, id, nav_flags);
        }

        ImVec2 text_pos = ImVec2(pos.x, pos.y + (size.y - label_size.y) * 0.5f);
...
bool ImGui::ListBox(const char* label, int* current_item, const char* const items[], int items_count, int height_items)
{
    ...
    if (height_items < 0)
        height_items = ImMin(items_count, 7);

    ImGuiStyle& style = g.Style;
    float text_h = GetTextLineHeightWithSpacing();
    float item_padding_y = 6.0f;                      // vertical extra space for each item
    float item_height = text_h + item_padding_y;      // per-item height used for layout/drawing

    ImVec2 size(0.0f, ImTrunc(item_height * height_items + style.FramePadding.y * 2.0f));
    if (!BeginListBox(label, size))
        return false;

    ImGuiWindow* window = g.CurrentWindow; // this is the child window created by BeginListBox
    ImDrawList* draw_list = window->DrawList;

    ImVec2 win_pos = window->Pos;
    ImVec2 win_size = window->Size;
    ImVec2 frame_min = win_pos;
    ImVec2 frame_max = ImVec2(win_pos.x + win_size.x, win_pos.y + win_size.y);
    float content_width = win_size.x;

    ImVec4 base_bg = GetStyleColorVec4(ImGuiCol_WindowBg);
    ImVec4 lighter_bg = ImVec4(
        ImClamp(base_bg.x + 0.02f, 0.0f, 1.0f),
        ImClamp(base_bg.y + 0.02f, 0.0f, 1.0f),
        ImClamp(base_bg.z + 0.02f, 0.0f, 1.0f),
        base_bg.w
    );

    ImU32 bg_col_u32 = GetColorU32(lighter_bg);
    ImU32 outline_col_u32 = IM_COL32(0, 0, 0, 255);
    ImU32 separator_col_u32 = GetColorU32(GetStyleColorVec4(ImGuiCol_Border));
    ImU32 text_col_u32 = GetColorU32(GetStyleColorVec4(ImGuiCol_Text));
    ImVec4 accent_color = ImVec4(0.61f, 0.67f, 0.95f, 1.0f);
    ImU32 accent_col_u32 = GetColorU32(accent_color);

    // draw background + outline for the whole listbox
    draw_list->AddRectFilled(frame_min, frame_max, bg_col_u32, 0.0f);
    draw_list->AddRect(frame_min, frame_max, outline_col_u32, 0.0f, 0, 1.0f);

    bool value_changed = false;
    float y = frame_min.y + style.FramePadding.y;
    float x = frame_min.x + style.FramePadding.x;
    float inner_width = content_width - style.FramePadding.x * 2.0f;
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        ImVec2 item_pos = ImVec2(x, y);
        ImVec2 item_size = ImVec2(inner_width, item_height);

        // create an invisible item to capture hover/click properly (prevents parent dragging)
        ImGui::SetCursorScreenPos(item_pos);
        char buf[64];
        ImFormatString(buf, IM_ARRAYSIZE(buf), "##listbox_item_%p_%d", (void*)label, i);
        ImGui::InvisibleButton(buf, item_size);
        ...
        // separator line between items (draw under the item)
        if (i < items_count - 1)
        {
            float line_y = y + item_height;
            draw_list->AddLine(ImVec2(frame_min.x + style.FramePadding.x, line_y),
                ImVec2(frame_max.x - style.FramePadding.x, line_y),
                separator_col_u32, 1.0f);
        }

        // draw text (active = accent color, else normal text color)
        ImU32 col = (i == *current_item) ? accent_col_u32 : text_col_u32;
        float text_y = y + (item_height - text_h) * 0.5f; // vertically center text
        draw_list->AddText(ImVec2(item_pos.x + 4.0f, text_y), col, item_text);
